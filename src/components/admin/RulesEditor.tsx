      ...
      <CardContent className="space-y-5">
        {fields.map(f => (
          <div key={f.key} className="flex items-center justify-between gap-4">
            ...
            <Input type="number" min={0} value={local[f.key]} onChange={e => set(f.key, e.target.value)} className="w-20 text-center" />
          </div>
        ))}
        <div className="flex items-center justify-between gap-4">
          ...
          <Input
            type="text"
            value={local.score_unit}
            onChange={e => setText('score_unit', e.target.value)}
